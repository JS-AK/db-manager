...
export const start = async (creds: MYSQL.ModelTypes.TDBCreds): Promise<void> => {
	await test("MYSQL-" + TEST_NAME, async (testContext) => {
		...
		await testContext.test(
			...
			async (testContext) => {
				...
				const id_sec = crypto.randomUUID();

				await testUserRepository.createOne({
					id,
					id_sec,
				...
				await testContext.test(
					...
					async () => {
						await testUserRepository.updateOneByPk([user.id, user.id_sec], {
				...
				await testContext.test(
					...
					async () => {
						await testUserRepository.deleteOneByPk([user.id, user.id_sec]);

						const result: [string, string] = [user.id, user.id_sec];
