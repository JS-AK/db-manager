 ...
 */
	...
	compareQuery = {
		...
		getOneByPk: <T = unknown | unknown[]>(primaryKey: T): { query: string; values: unknown[]; } => {
			if (!this.primaryKey) { throw new Error("Primary key not specified"); }
		...
		updateOneByPk: <T = unknown | unknown[]>(
			primaryKeyValue: T,
			updateFields: SharedTypes.TRawParams = {},
	...
	async getOneByPk<T = unknown | unknown[]>(primaryKey: T) {
	...
	async updateOneByPk<Q extends pg.QueryResultRow, T = unknown | unknown[]>(
		primaryKeyValue: T,
		updateFields: SharedTypes.TRawParams = {},
		updateOptions?: { returningFields?: string[]; },
	) {
