...
describe("BaseTable", () => {
	...
	describe("compareQuery.createOne", () => {
		it("should build INSERT query and values", () => {
			...
			expect(result.query).toBe("INSERT INTO \"users\" (\"age\",\"name\") VALUES ($1,$2)  RETURNING *;");
		...
		it("should apply onConflict from insertOptions", () => {
			const table = createTable(usersSchema, {
				insertOptions: { onConflict: "ON CONFLICT DO NOTHING" },
			});
			const result = table.compareQuery.createOne(
				{ name: "John" },
				{ returningFields: ["id"] },
			);
	...
	describe("compareQuery.createMany", () => {
		...
		it("should build INSERT for multiple rows", () => {
			...
			expect(result.query).toBe(
				"INSERT INTO \"users\" (\"age\",\"name\") VALUES ($1,$2),($3,$4)  RETURNING *;",
			);
	...
	describe("compareQuery.deleteByParams", () => {
		it("should build DELETE with search conditions", () => {
			...
			expect(result.query).toBe("DELETE FROM \"users\" WHERE (\"age\" > $1 AND \"published\" = $2);");
	...
	describe("compareQuery.deleteOneByPk", () => {
		it("should build DELETE by primary key", () => {
			...
			expect(result.query).toBe("DELETE FROM \"users\" WHERE \"id\" = $1 RETURNING \"id\";");
	...
	describe("compareQuery.getOneByParams", () => {
		it("should build SELECT with LIMIT 1", () => {
			...
			expect(result.query).toBe(
				"SELECT \"id\", \"name\" FROM \"users\" WHERE (\"published\" = $1) LIMIT 1 OFFSET 0;",
			);
		...
		it("should support quoted SQL identifiers from schema", () => {
			...
			expect(result.query).toBe(
				"SELECT \"typeID\", \"typeName\" FROM \"invTypes\" WHERE (\"typeID\" = $1) LIMIT 1 OFFSET 0;",
			);
	...
	describe("compareQuery.getArrByParams", () => {
		it("should build SELECT with pagination and order", () => {
			...
			expect(result.query).toBe(
				"SELECT \"id\", \"name\" FROM \"users\" WHERE (\"published\" = $1) ORDER BY \"name\" ASC LIMIT 10 OFFSET 5;",
			);
			expect(result.values).toEqual([true]);
		});
	...
	describe("compareQuery.getCountByParams", () => {
		it("should build COUNT query", () => {
			...
			expect(result.query).toBe("SELECT COUNT(*) AS count FROM \"users\" WHERE (\"published\" = $1);");
			expect(result.values).toEqual([true]);
		});
	});
	...
	describe("compareQuery.getOneByPk", () => {
		it("should build SELECT by primary key", () => {
			...
			expect(result.query).toBe("SELECT * FROM \"users\" WHERE \"id\" = $1 LIMIT 1;");
	...
	describe("compareQuery.getCountByPks", () => {
		it("should build COUNT for scalar primary keys", () => {
			...
			expect(result.query).toBe("SELECT COUNT(*) AS count FROM \"users\" WHERE \"id\" = ANY ($1);");
			expect(result.values).toEqual([[1, 2, 3]]);
		});

		it("should build COUNT for composite primary keys", () => {
			const table = createTable({
				...usersSchema,
				primaryKey: ["tenant_id", "user_id"],
				tableFields: ["tenant_id", "user_id", "name"],
			});
			const result = table.compareQuery.getCountByPks([[1, 10], [2, 20]]);

			expect(result.query).toBe(
				"SELECT COUNT(*) AS count FROM \"users\" WHERE ((\"tenant_id\" = $1 AND \"user_id\" = $2) OR (\"tenant_id\" = $3 AND \"user_id\" = $4));",
			);
			expect(result.values).toEqual([1, 10, 2, 20]);
		});
	});

	describe("compareQuery.getCountByPksAndParams", () => {
		it("should combine search params and primary key filter", () => {
			const table = createTable();
			const result = table.compareQuery.getCountByPksAndParams(
				[1, 2, 3],
				{ $and: { published: true } },
			);

			expect(result.query).toBe(
				"SELECT COUNT(*) AS count FROM \"users\" WHERE (\"published\" = $1) AND \"id\" = ANY ($2);",
			);
			expect(result.values).toEqual([true, [1, 2, 3]]);
		});
	});
	...
	describe("compareQuery.streamArrByParams", () => {
		it("should build the same query shape as getArrByParams", () => {
			const table = createTable();
			const result = table.compareQuery.streamArrByParams(
				{ $and: { published: true } },
				["id"],
				{ limit: 5, offset: 0 },
			);
			...
			expect(result.query).toBe(
				"SELECT \"id\" FROM \"users\" WHERE (\"published\" = $1) LIMIT 5 OFFSET 0;",
			);
		});
	});
	...
	describe("compareQuery.updateByParams", () => {
		it("should build UPDATE with WHERE and SET clauses", () => {
			...
			const result = table.compareQuery.updateByParams(
				{ $and: { id: 1 }, returningFields: ["id", "name"] },
				{ age: 31, name: "John" },
			);

			expect(result.query).toBe(
				"UPDATE \"users\" SET \"age\" = $2,\"name\" = $3, \"updated_at\" = NOW() WHERE (\"id\" = $1) RETURNING \"id\",\"name\";",
			);
			expect(result.values).toEqual([1, 31, "John"]);
		});
