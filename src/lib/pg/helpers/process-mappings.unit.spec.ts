...
describe("processMappings", () => {
	...
	it("should process $@> operator", () => {
		const queryArray: Types.TField[] = [];
		const values: unknown[] = [];
		const func = processMappings.get("$@>");

		func?.("field", { "$@>": { a: 1 } }, queryArray, values);
		expect(queryArray).toEqual([{ key: "field", operator: "@>" }]);
	...
	it("should process $&& operator", () => {
		...
		func?.("field", { "$&&": [1, 2] }, queryArray, values);
		expect(queryArray).toEqual([{ key: "field", operator: "&&" }]);
	...
	it("should process $@ operator", () => {
		...
		func?.("field", { "$@": "value" }, queryArray, values);
		expect(queryArray).toEqual([{ key: "field", operator: "@" }]);
		expect(values).toEqual(["value"]);
	});
	...
	it("should process $~ operator", () => {
		...
		func?.("field", { "$~": "pattern" }, queryArray, values);
		expect(queryArray).toEqual([{ key: "field", operator: "~" }]);
	...
	it("should process $? operator", () => {
		...
		func?.("field", { "$?": "key" }, queryArray, values);
		expect(queryArray).toEqual([{ key: "field", operator: "?" }]);
	...
	it("should process $jsonb operator", () => {
		const queryArray: Types.TField[] = [];
		const values: unknown[] = [];
		const func = processMappings.get("$jsonb");

		func?.("field", { $jsonb: { a: 1 } }, queryArray, values);
		expect(queryArray).toEqual([{ key: "field", operator: "=" }]);
		expect(values).toEqual([{ a: 1 }]);
	});
