...
> {
	...
	constructor(data: Types.TDomain<M>) {
		if (!(data.model instanceof Model)) {
			throw new Error("You need pass data.model extended of MYSQL.Model.BaseView");
		}
	 ...
	 */
	setClientInBaseClass(client: mysql.Pool | mysql.PoolConnection | mysql.Connection): BaseView<Model, BVG> {
