...
describe("getFieldsToSearch", () => {
	...
	it("should handle simple queryArray", () => {
		...
		expect(result.searchFields).toBe(" WHERE (name = ?)");
	});
	...
	it("should handle operatorMappings", () => {
		...
		expect(result.searchFields).toBe(" WHERE ((age BETWEEN ? AND ?) AND (status IN (?)))");
	});
	...
	it("should handle queryOrArray", () => {
		...
		expect(result.searchFields).toBe(" WHERE (is_active = ? AND ((name = ?) OR (email LIKE ?)))");
	});
	...
	it("should handle a complex query", () => {
		...
		expect(result.searchFields).toBe(" WHERE ((category = ?) AND (price BETWEEN ? AND ?) AND ((tag IN (?)) OR (LOWER(description) LIKE LOWER(?))))");
